...
Requires: mmcal.py (place in same folder), watat_years.json
...
class MyanmarConverterApp(tk.Tk):
    def __init__(self):
        ...
        # --- Load watat years data ---
        self.watat_years = self.load_watat_years()
    ...
    def load_watat_years(self):
        """Load watat years from JSON file"""
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            watat_file = os.path.join(script_dir, 'watat_years.json')
            with open(watat_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                return data.get('watat_years', [])
        except Exception as e:
            print(f"Error loading watat years: {e}")
            return []

    def is_watat_year(self, myanmar_year):
        """Check if a Myanmar year is a watat year (has intercalary month)"""
        return myanmar_year in self.watat_years
    ...
    def _en_month_to_mm(self, en_month, myanmar_year=None):
        """
        ...
        # Special handling for Waso in watat years
        if en_month == 'Waso' and myanmar_year and self.is_watat_year(myanmar_year):
            return 'ဒုတိယ ဝါဆို'
    ...
    def convert(self):
        ...
        try:
            ...
            # Extract Myanmar year number for checking watat years
            myanmar_year = int(year_num_str)
            mm_month = self._en_month_to_mm(en_month, myanmar_year) # Pass Myanmar year for watat check
            mm_year_num_str = self._int_to_mm_digit(myanmar_year)
