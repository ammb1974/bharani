...
class Chart(DotDict):
    ...
    def svg_toplevel(self, content: str | list[str]) -> str:
        """Generate an SVG toplevel element with sensible defaults.

        Args:
            content: The content to be included in the SVG toplevel

        Returns:
            An SVG toplevel element as a string
        """
        return svg(
            content,
            height=self.height,
