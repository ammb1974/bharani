...
class MyanmarConverterApp(tk.Tk):
    ...
    def build_ui(self):
        ...
        month_names = [m for m in months[1:]]
        self.month_cb = ttk.Combobox(frm, values=month_names, state="readonly", textvariable=self.month_var, width=12)
    ...
    def update_days(self):
        ...
        if not month_name:
            ...
            try:
                ...
                # fallback: if month_name already in english lower-case
                month_index = datetime.now().month
    ...
    def convert(self):
        try:
            y = int(self.year_var.get())
            m = months.index(self.month_var.get())
