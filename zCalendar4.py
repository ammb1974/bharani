...
Handles intercalary months (e.g., ဒုတိယ ဝါဆို).
"""
import sys
import calendar
...
class MyanmarConverterApp(tk.Tk):
    def __init__(self):
        ...
        # --- CORRECTED: Create a map for Myanmar month names (English to Burmese) ---
        # This uses the 'months' and 'monthsMM' lists from mmcal
        self.mm_month_map = {}
    ...
    def _en_month_to_mm(self, en_month):
        """Maps English Myanmar month name to Burmese month name, including intercalary months."""
        # Check intercalary months first
        if en_month == 'Second Waso':
            return 'ဒုတိယ ဝါဆို'
        
        # Then use the generated map
    ...
    def convert(self):
        ...
        try:
            ...
            # e.g., "14 Tazaungmon, 1387" or "29 Second Waso, 1387"
            en_str = to_myanmar(dt, lang='en')
            
            # 2. Parse the string robustly
            ...
            if cumulative_day <= 15:
                ...
                mm_phase = "လဆုတ်"
                mm_day_str = self._int_to_mm_digit(cumulative_day - 15)
            
            # 5. Construct the final Burmese string
            final_str = f"{mm_month} {mm_phase} {mm_day_str} ရက် {mm_year_num_str}"
            self.result_var.set(final_str)

            # 6. Set the moon age (the cumulative day)
